...
Public Class Datasheet
        ...
        For i As Integer = 0 To numero_DS - 1
            Try
                ...
                lista_DS_sel_name = DS_lista(i, 1)
                    ...
                    form_Archivio.Guna2ProgressBar1.Value = Math.Round(i / (numero_DS - 1) * 100, 0)
                    Label58.Text = "Updating " & DS_lista(i, 1) & ". Process at " & Math.Round(i / (numero_DS - 1) * 100, 0) & " %"
                    form_Archivio.Label22.Location = New System.Drawing.Point(450, 550)
                Application.DoEvents()
    ...
    Public Sub stampa_code(n)
        ...
        'Aggiorno i punti delle variabili di pressione e portata
        Q1_DS = Q1.Text
        Q2_DS = Q2.Text
        Q3_DS = Q3.Text
        Q4_DS = Q4.Text
        ...
        Else 'CASO DI CICLO DI STAMPA
            ...
            If num_conf_sel = 1 And PrintAll = 0 Then
                ...
                '*************************************************SQL SERVER AGGIUNGO******************************************************************************
                ERP1(n)
                If DS_totale_dati(n, 87) = True And Datasheet_print_mode = 1 Or ciclo_stampa_catalogo = 1 Then 'solo se c'è la spunta nell'archivio carico sul sito 
                    If ciclo_stampa_catalogo = 0 Then
    ...
    Private Sub Guna2Button3_Click(sender As Object, e As EventArgs) Handles Salva_DB.Click
        ...
        If blocca_add = 1 Then ' nel caso il datasheet esistesse gia'
            ...
            If Yes_No_Warning = 1 Then ' in caso di risposta affermaiva allora aggiorno
                ...
                If descrizione_fan <> Nothing And descrizione_fan <> "" Then
                    For Each d In Directory.GetDirectories(folders_directoryDS & "\" & descrizione_fan)
                        Directory.Delete(d, True)
                    Next

                    Try
                        File.Delete(folders_directoryDS & "\" & descrizione_fan & "\" & descrizione_fan & ".pdf")
                    Catch ex As Exception
            ...
            Try
                ...
                If System.IO.File.Exists(file_name_MP) = False Then
                    ...
                    IO.File.Copy(modulo_PJ_PF, file_name_MP, True) 'copy(dalla cartella, alla cartella con nome del file)
                End If
    ...
    Private Sub StampaDS_CheckedChanged(sender As Object, e As EventArgs) Handles StampaDS.CheckedChanged
        If StampaDS.Checked = True Then
            Note_print = 1
        Else
            Note_print = 0
        End If
